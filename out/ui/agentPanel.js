...
class AgentPanel {
    ...
    setupEventHandlers() {
        ...
        // Message handling
        this.disposables.push(this.panel.webview.onDidReceiveMessage((message) => this.handleMessage(message)));
    }
    ...
    updateContent(agent, content, type = 'info') {
        if (this.panel) {
            this.panel.webview.postMessage({
                command: 'updateContent',
                agent,
    ...
    updateBadge(agent, count, type = 'info') {
        if (this.panel) {
            this.panel.webview.postMessage({
                command: 'updateBadge',
                agent,
    ...
    async exportContent(agent, content) {
        if (!content.trim()) {
